#!/usr/bin/env python3
"""
Fetch Hong Kong stock data for multi-market-stock-analysis skill.

Usage:
    python3 fetch_hk_data.py --ticker 00700.HK --days 252

Note:
    Ticker format: XXXXX.HK (e.g., 00700.HK for Tencent, 09988.HK for Alibaba)
...
def parse_args():
    ...
    parser.add_argument("--ticker", type=str, required=True, help="Stock ticker (e.g., 00700.HK)")
    parser.add_argument("--days", type=int, default=252, help="Number of trading days to fetch (default: 252)")
    return parser.parse_args()

def fetch_hk_data(ticker, days=252):
    """
    ...
    # Normalize ticker format (ensure .HK suffix)
    if not ticker.endswith(".HK"):
        # Try to format as XXXXX.HK
...
def main():
    args = parse_args()
    result = fetch_hk_data(args.ticker, args.days)
